...
void joinch(){
  ...
  strcat(request,authtok);
    ...
    strcpy(fresp, request);
    char err[]="Error";
    char iferr[MAX],errdec[MAX];
    GetOIJSON(fresp, "type",iferr);
    GetOIJSON(fresp, "content",errdec);
    printf("%s is iferr\n%s is errde\n",iferr,errdec);
    if (strcmp(err, iferr)==0) {
        printf("%s:\n%s\n",iferr,errdec);
        mainmenu();
    }
    shutdown(client_socket, SHUT_RDWR);
...
void logi(){
  ...
  strcat(request,pf);
    ...
    shutdown(client_socket, SHUT_RDWR);
    if(strcmp(authtoken, iferr)==0){
        printf("Successfully Logged in\n");
        strncpy(authtok, errdec,24);
    }
  mainmenu();
