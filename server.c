...
int refr(char authtoken[],int client_socket,int server_socket){
    ...
    if((users=fopen(usn, "r"))!=NULL){
        ...
        cJSON *point= cJSON_GetObjectItemCaseSensitive(message, "point");
    ...
    cJSON *mems = NULL;
        ...
        fclose(channels);
            ...
            if((users=fopen(usn, "r"))!=NULL){
                ...
                fgets(msg, MAX, users);
                cJSON *message = cJSON_Parse(msg);
                cJSON *point=cJSON_CreateNumber(po);
...
int joinch(char channel[], char chname[],char authtoken[],int client_socket,int server_socket){
   strcat(channel, ".channel.json");
    ...
    cJSON *messages = NULL;
        ...
        cJSON *sender=NULL;
            cJSON *mem=NULL;
            mem=cJSON_GetObjectItem(curde, "members");
        ...
        cJSON_AddItemToObject(eachmsg, "content", msg);
        cJSON_AddItemToArray(messages, eachmsg);
            cJSON *memun=cJSON_CreateString(usn);
            cJSON_AddItemToArray(mem, memun);
        char* wtp=cJSON_PrintUnformatted(curde);
        fprintf(channels, "%s",wtp);
        fclose(channels);
        cJSON *content = NULL;
        ...
        close(server_socket);
            return 0;
                ...
                cJSON *content = NULL;
                  cJSON *type = NULL;
                  cJSON *err = cJSON_CreateObject();
                  content = cJSON_CreateString("Name not available.");
                  type= cJSON_CreateString("Error");
                  cJSON_AddItemToObject(err, "type", type);
                  cJSON_AddItemToObject(err, "content", content);
                  char*errors=cJSON_PrintUnformatted(err);
...
int createch(char channel[], char chname[],char authtoken[],int client_socket,int server_socket){
    ...
    cJSON *messages = NULL;
    cJSON *name = NULL;
    cJSON *chdetails = cJSON_CreateObject();
        cJSON *msg=NULL;
        cJSON *sender=NULL;
        printf("%s\n",chname);
       name = cJSON_CreateString(chname);
        messages= cJSON_CreateArray();
            cJSON *mem=NULL;
            mem=cJSON_CreateArray();
       cJSON_AddItemToObject(chdetails, "name", name);
        cJSON_AddItemToObject(chdetails, "messages", messages);
            cJSON_AddItemToObject(chdetails, "members", mem);
        sender=cJSON_CreateString("server");
        char wtwam[MAX];
            if((un=fopen("username.txt", "r"))!=NULL){
                fgets(usn, MAX, un);
        ...
        char* wtp=cJSON_PrintUnformatted(chdetails);
        fprintf(channels, "%s",wtp);
        fclose(channels);
            }
        cJSON *content = NULL;
...
int reg(char user[], char uname[],char pass[],int client_socket,int server_socket){
  ...
  if((users=fopen(user, "a"))!=NULL){
    cJSON *username = NULL;
    cJSON *password = NULL;
    cJSON *userdetails = cJSON_CreateObject();
      username = cJSON_CreateString(uname);
      printf("%s\n%s\n",uname,pass);
    password= cJSON_CreateString(pass);
       cJSON_AddItemToObject(userdetails, "username", username);
    cJSON_AddItemToObject(userdetails, "password", password);
      cJSON_AddNumberToObject(userdetails, "point", 0);
        char* wtp=cJSON_PrintUnformatted(userdetails);
        fprintf(users, "%s",wtp);
        fclose(users);
        cJSON *content = NULL;
        cJSON *type = NULL;
...
close(client_socket);
    ...
    cJSON *content = NULL;
        cJSON *type = NULL;
        cJSON *err = cJSON_CreateObject();
        content = cJSON_CreateString("This username is not available.");
        type= cJSON_CreateString("Error");
        cJSON_AddItemToObject(err, "type", type);
        cJSON_AddItemToObject(err, "content", content);
        char*errors=cJSON_PrintUnformatted(err);
...
int logi(char user[], char uname[],char pass[],int client_socket,int server_socket){
  ...
  if((users=fopen(user, "r"))!=NULL){
      ...
      fgets(msg, MAX, users);
      cJSON *message = cJSON_Parse(msg);
      cJSON *password= cJSON_GetObjectItemCaseSensitive(message, "password");
      if (strcmp(password->valuestring, pass)==0) {
        ...
        cJSON *type = NULL;
        cJSON *err = cJSON_CreateObject();
           content = cJSON_CreateString(authtok);
      ...
      cJSON *content = NULL;
          cJSON *type = NULL;
          cJSON *err = cJSON_CreateObject();
          content = cJSON_CreateString("Username is not valid.");
          type= cJSON_CreateString("Error");
          cJSON_AddItemToObject(err, "type", type);
          cJSON_AddItemToObject(err, "content", content);
          char*errors=cJSON_PrintUnformatted(err);
